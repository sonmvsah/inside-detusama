...
module.exports = function (hexo) {
  ...
  const pluginManifest = loadManifest([
    path.join(hexo.theme_dir, "lib/plugins"),
  ]);
  ...
  hexo.on("generateBefore", function () {
    ...
    const result = utils.parseConfig(configSchema, theme, {
      $email: email,
      $feed: feed,
      $copyright: `&copy; ${new Date().getFullYear()} • <a href="${site.url}">${
        site.author
      }</a>`,
    ...
    hexo.extend.injector.register(
      "head_end",
      `<style is="theme">${css(utils.flattenObject(result.appearance))}</style>`
    );
    // theme.js
    hexo.extend.injector.register(
      "head_end",
      `<script src="${urlFor.call(hexo, manifest.theme)}"></script>`
    );
    ...
    if (result.footer.powered)
      result.footer.powered = __(
        "footer.powered",
        '<a href="https://maivietson.com" target="_blank" rel="external nofollow noopener">mVs</a>'
      );
    if (result.footer.theme)
      result.footer.theme =
        __("footer.theme") +
        ' - <a href="https://blog.maivietson.com/" target="_blank" rel="external nofollow noopener">Inside</a>';
    ...
    result.runtime = {
      ...
      hasComments: !!(
        result.comments ||
        (result.plugins && result.plugins.comments)
      ),
      ...
      uriReplacer: (() => {
        ...
        return (src, assetPath) => {
          ...
          return /^(\/\/?|http|data\:image)/.test(src)
            ? src
            : assetsFn(`${assetPath}${src}`);
        };
      })(),
    };
