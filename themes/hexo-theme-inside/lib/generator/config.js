...
module.exports = function (locals) {
  ...
  config.color = [
    ...
    parseBackground(background).color ||
      (theme.pwa && theme.pwa.theme_color) ||
      []
  );
  ...
  config.index = [...locals.posts, ...locals.pages].find((i) => i.link === "")
    ? "index"
    : "page";
    ...
    config.routes.pages = [
      ...locals.pages.reduce((set, post) => {
        ...
        const link = post.link
          ...
          .filter((i) => i)
          .map((partial, i) =>
            i === 0 ? partial : ":" + String.fromCharCode(97 + i)
          )
          .join("/");
        set.add(link);
        return set;
